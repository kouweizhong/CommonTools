...
namespace CommonTools.Win.Encrypte
{
    public partial class EncrypteForm : BaseForm
    {
        ...
        private void PicSubmitMd5_Click(object sender, EventArgs e)
        {
        ...
        private void PicSubmitSha_Click(object sender, EventArgs e)
        {
        ...
        private void PicSubmitDes_Click(object sender, EventArgs e)
        {
        ...
        private void PicSubmitRc2_Click(object sender, EventArgs e)
        {
        ...
        private void PicSubmitAes_Click(object sender, EventArgs e)
        {
        ...
        private void PicSubmitDes3_Click(object sender, EventArgs e)
        {
        ...
        private void BtnBrowser_Click(object sender, EventArgs e)
        {
            ...
            dialog.Filter = string.Format("jpg (*.jpg)|*.jpg|png (*.png)|*.png");
            dialog.FilterIndex = 1;
            dialog.RestoreDirectory = true;
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                float i = dialog.OpenFile().Length;
                if (i / 1024 > PublicFields.ImageSize)//图片上限
                {
                    MessageBox.Show(string.Format("图片超过{0}KB，请选择小图片！", PublicFields.ImageSize));
                    return;
                }
        ...
        private void PicSubmitBase64_Click(object sender, EventArgs e)
        {
            ...
            {
                LoadingEncryote.ShowWaitForm();
                LoadingEncryote.SetWaitFormCaption("正在转换中,请稍后......");
                try
                {
                    image = Image.FromFile(filename);
                    TxtBase64.Text = image.ImageToBase64(image.RawFormat);
                    TxtBase64Html.Text = @"<img src='data:image/png;base64,Base64的值' />";
                    TxtBase64Css.Text = @"#id { background-image: url('data:image/png;base64,Base64的值'); }";
                    LoadingEncryote.CloseWaitForm();
                }
                catch (Exception)
                {
                    LoadingEncryote.CloseWaitForm();
        ...
        /// Author  : 俞立钢
        /// Company : 绍兴标点电子技术有限公司
        /// Created : 2014-12-12 11:20:15
        public void ExportImage(Image image)
        {
            FolderBrowserDialog dialog = new FolderBrowserDialog();
            dialog.Description = string.Format("设置保存图片的路径！");
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                string fileName = dialog.SelectedPath + "\\Base64" + image.GetExtension();
                image.Save(fileName);
            }
        }
