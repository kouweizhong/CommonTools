        ...
        private void PicSubmitBase64_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(base64))
            {
                try
                {
                    ...
                    TxtBase64Html.Text = string.Format(@"<img src='data:image/png;base64,Base64的值' />");
                    TxtBase64Css.Text = string.Format(@"#id { background-image: url('data:image/png;base64,Base64的值'); }");
                }
                catch (Exception)
                {
