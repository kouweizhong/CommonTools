...
namespace CommonTools.Common
{
    public static class PublicFunc
    {
        ...
        public static string GetMd5(this string value)
        {
            ...
            string sd = BitConverter.ToString(mClassMd5.ComputeHash(Encoding.Default.GetBytes(value)));
            string mstrEncrypt = sd.Replace("-", "");
            string s = FormsAuthentication.HashPasswordForStoringInConfigFile(value,
        ...
        public static string GetSha1(this string value, char replace)
        {
            SHA1 sha = new SHA1CryptoServiceProvider();
            string bits = BitConverter.ToString(sha.ComputeHash(Encoding.Default.GetBytes(value)));
            string sha1 = bits.Replace('-', replace);
            return sha1;
        }
