...
namespace CommonTools.Common
{
    public static class PublicFunc
    {
        ...
        public static string EncrypteDes(this string value, string key)
        {
            ...
            byte[] m_btEncryptString = Encoding.Default.GetBytes(value);
            MemoryStream m_stream = new MemoryStream();
            CryptoStream m_cstream = new CryptoStream(m_stream, m_DESProvider.CreateEncryptor(Encoding.Default.GetBytes(key), m_btIV), CryptoStreamMode.Write);
            m_cstream.Write(m_btEncryptString, 0, m_btEncryptString.Length);
            m_cstream.FlushFinalBlock();
            string m_strEncrypt = Convert.ToBase64String(m_stream.ToArray());
