...
namespace CommonTools.Win.Encrypte
{
    public partial class EncrypteForm : BaseForm
    {
        ...
        {
            TxtNumber.Text = PublicFields.Sha1Number.ToString("D");
            labelControl3.Text = string.Format("(默认加密{0}次)", PublicFields.Sha1Number);
            labelControl15.Text = string.Format("(最高为{0}位密文,只能填写数字)", PublicFields.Sha1EndNumber);
        }
